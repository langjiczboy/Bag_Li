...
for num_B=1:length(B_vec)
    ...
    for num_a_4=1:length(a_4_vec)
        ...
        for num_Delta=1:length(Delta_vec)
            ...
            Omega=(Omega_u+Omega_d+Omega_s)+3/(4*pi^2)*(1-a_4)*(mu_b/3)^4+B_eff-3/pi^2*Delta^2*mu_b^2;
            %% n
            
            n_u=N_c/(3*pi^2)*nu^3-1/pi^2*(1-a_4)*(mu_b/3)^3+(6*Delta^2*mu_b)/pi^2;
            n_d=N_c/(3*pi^2)*nu^3-1/pi^2*(1-a_4)*(mu_b/3)^3+(6*Delta^2*mu_b)/pi^2;
            n_s=N_c/(3*pi^2)*nu^3-1/pi^2*(1-a_4)*(mu_b/3)^3+(6*Delta^2*mu_b)/pi^2;
...
save([bag_main_path,'\Bag_Li_CFL a_4 ', num2str(a_4),' .mat'],'bag_Li_CFL_mat_set')
figure
